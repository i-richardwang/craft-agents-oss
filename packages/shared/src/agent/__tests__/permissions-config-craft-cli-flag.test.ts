...
const originalConfigDir = process.env.DATAPILOT_CONFIG_DIR
...
afterEach(() => {
  ...
  if (originalConfigDir === undefined) delete process.env.DATAPILOT_CONFIG_DIR
  else process.env.DATAPILOT_CONFIG_DIR = originalConfigDir
...
describe('permissions config craft-agents-cli feature flag', () => {
  it('skips compiling craft-agent bash allowlist patterns when feature is disabled', () => {
    ...
    try {
      process.env.DATAPILOT_CONFIG_DIR = tempConfigDir
